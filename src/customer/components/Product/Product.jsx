...
const filters = [
  {
    ...
    options: [
      ...
      { value: "bleu", label: "bleu", checked: false },
      { value: "noir", label: "noir", checked: false },
      { value: "vert", label: "vert", checked: false },
...
export default function Product() {
  ...
  const handleFilter = (value, sectionId) => {
    ...
    // Récupérer les valeurs existantes pour la section donnée
    let filterValues = searchParams.get(sectionId);

    // Si des valeurs existent déjà pour cette section, on les transforme en tableau
    filterValues = filterValues ? filterValues.split(",") : [];

    // Vérifier si la valeur est déjà présente dans les filtres
    if (filterValues.includes(value)) {
      // Si elle est présente, on la supprime
      filterValues = filterValues.filter((item) => item !== value);

      // Si après suppression il n'y a plus de valeurs, on supprime la section du query
      if (filterValues.length === 0) {
        searchParams.delete(sectionId);
      }
    } else {
      // Si la valeur n'est pas présente, on l'ajoute au tableau
      filterValues.push(value);
    }

    // Si des valeurs existent après la modification, on met à jour l'URL
    if (filterValues.length > 0) {
      searchParams.set(sectionId, filterValues.join(","));
    }

    // Générer la nouvelle query string
    ...
    <div className="bg-white">
      <div>
        ...
        <main className="mx-auto  px-4 sm:px-6 lg:px-8">
          <div className="flex items-baseline justify-end border-b border-gray-200 pb-6 pt-5">
            ...
            <div className="grid grid-cols-1 gap-x-2 gap-y-10 lg:grid-cols-6">
              ...
              <form className="hidden lg:block">
                ...
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg opacity-50 font-bold">Filtres</h3>
                  <FilterListIcon />
                </div>
                {/* <ul
                  role="list"
                        ...
                        {section.options.map((option, optionIdx) => (
                          <div key={option.value} className="flex items-center">
                            <input
                              ...
                              onChange={(e) =>
                                handleFilter(option.value, section.id)
                              } // Appel de la fonction handleFilter
                              className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                            />
                            <label
          ...
          <section className="w-full px-[3.5rem] py-3">
